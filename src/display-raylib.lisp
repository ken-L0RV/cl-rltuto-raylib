...
(defun handle-keys ()
  (let ((action nil))
    (cond ((or (is-key-pressed :key-right) (is-key-down :key-l)) (setf action (list :movement (cons 1 0))))
          ((or (is-key-pressed :key-down) (is-key-down :key-j)) (setf action (list :movement (cons 0 1))))
          ((or (is-key-pressed :key-left) (is-key-down :key-h)) (setf action (list :movement (cons -1 0))))
          ((or (is-key-pressed :key-up) (is-key-down :key-k)) (setf action (list :movement (cons 0 -1))))
          ((is-key-pressed :key-g) (setf action (list :pickup T)))
          ((and (is-key-down :key-d) (is-key-pressed :key-one)) (setf action (list :drop-item 0)))
          ((and (is-key-down :key-d) (is-key-pressed :key-two)) (setf action (list :drop-item 1)))
          ((and (is-key-down :key-d) (is-key-pressed :key-three)) (setf action (list :drop-item 2)))
          ((and (is-key-down :key-d) (is-key-pressed :key-four)) (setf action (list :drop-item 3)))
          ((is-key-pressed :key-one) (setf action (list :use-item 0)))
          ((is-key-pressed :key-two) (setf action (list :use-item 1)))
          ((is-key-pressed :key-three) (setf action (list :use-item 2)))
          ((is-key-pressed :key-four) (setf action (list :use-item 3)))
          ((is-key-pressed :key-kp-0) (setf action (list :wait T)))
          ((is-key-pressed :key-c) (setf action (list :reveal-scene T)))
    )
    action))
    ...
    (with-window (*screen-width* *screen-height* "roguelike tuto w/ raylib in cl")
      ...
      (loop
        ...
        (with-drawing
          ...
          (run-update-entity-visibility)
          (run-draw-visible-corpses)
          (run-draw-visible-creatures)

          (run-draw-visible-items)
